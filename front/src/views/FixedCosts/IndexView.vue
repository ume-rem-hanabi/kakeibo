<template>
                ...
                <div class="d-flex align-center justify-space-between">
                    <div>
                        <h1 class="text-h4 font-weight-bold mb-2">
                            <v-icon left size="large" color="primary">mdi-cash-multiple</v-icon>
                            固定費管理
                        </h1>
                ...
                <v-card>
                    <v-card-text>
                        <div class="d-flex align-center justify-space-between">
                            ...
                            <v-icon size="80" color="grey-lighten-2">mdi-format-list-numbered</v-icon>
                        </div>
                    </v-card-text>
                </v-card>
            </v-col>
        ...
        <v-row>
            <v-col cols="12">
                <v-card>
                    ...
                    <v-card-text v-else-if="fixedCostStore.fixedCosts.length === 0" class="text-center py-8">
                        <v-icon size="80" color="grey-lighten-2" class="mb-4">mdi-inbox</v-icon>
                        <p class="text-h6 text-grey">固定費が登録されていません</p>
                        <v-btn color="primary" class="mt-4" prepend-icon="mdi-plus" @click="openCreateDialog">
                            最初の固定費を登録
                        </v-btn>
                    </v-card-text>

                    <v-list v-else lines="three">
                        <template v-for="(item, index) in fixedCostStore.fixedCosts" :key="item.id">
                            <v-list-item>
                                ...
                                <v-list-item-subtitle class="mt-1">
                                    <v-chip size="small" :color="item.category?.color" class="mr-2">
                                        {{ item.category?.name }}
                                    </v-chip>
                            ...
                            <v-divider v-if="index < fixedCostStore.fixedCosts.length - 1"></v-divider>
                        </template>
                    </v-list>
                </v-card>
            </v-col>
    ...
    <!-- 登録・編集ダイアログ -->
    <FixedCostDialog v-model="dialogOpen" :edit-item="editItem" :categories="fixedCostStore.categories"
        @saved="handleSaved" />
